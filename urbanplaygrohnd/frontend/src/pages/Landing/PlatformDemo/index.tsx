  ...
  useGSAP(() => {
    ...
    if (headerRef.current) {
      ...
      tl.from(headerRef.current.querySelector(`.${styles.sectionTitle}`), {
        ...
        headerRef.current.querySelector(`.${styles.sectionSubtitle}`),
        {
          opacity: 0,
          y: 20,
          duration: 0.6,
    ...
    const cards =
      containerRef.current ?
        gsap.utils.toArray<HTMLElement>('.demo-card', containerRef.current)
      : [];
  ...
  return (
    <section className={`${styles.platformDemo} platformDemo`}>
      <div ref={headerRef} className='demo-header'>
        <h2 className={styles.sectionTitle}>See uplayground in Action</h2>
      ...
      <div ref={containerRef} className={styles.demoWrapper}>
        <div className={`${styles.cardsContainer} cards-container`}>
          {demoCards.map((card, index) => {
            ...
            return (
              <div key={card.id} className={`${styles.demoCard} demo-card`} data-card-index={index}>
