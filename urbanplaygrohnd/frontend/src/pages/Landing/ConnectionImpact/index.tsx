...
const ConnectionImpact = () => {
  ...
  const statsRef = useRef<StatsState>({ connections: 0, events: 0, returnRate: 0 });
  const [animatedStats, setAnimatedStats] = useState<StatsState>({
    connections: 0,
    events: 0,
    returnRate: 0,
  });
  ...
  useGSAP(
    () => {
      ...
      const connections = gsap.utils.toArray<HTMLElement>('.network-connection');
    ...
    <section ref={containerRef} className={`${styles.connectionImpact} connectionImpact`}>
      <div className={styles.container}>
        <div className='impact-header'>
          ...
          <p className={styles.sectionSubtitle}>Real connections that last beyond the event</p>
        </div>
        ...
        <div className={styles.statsGrid}>
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <div className={styles.statNumber}>{animatedStats.connections.toLocaleString()}</div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <div className={styles.statNumber}>{animatedStats.events.toLocaleString()}</div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <div className={styles.statNumber}>{animatedStats.returnRate}%</div>
        ...
        <div className={styles.networkVisualization}>
          <svg viewBox='0 0 800 400' className={styles.networkSvg}>
            ...
            {/* Profile avatars */}
            <text x='200' y='210' textAnchor='middle' fill='white' fontSize='20'>
              👤
            </text>
            <text x='400' y='110' textAnchor='middle' fill='black' fontSize='20'>
              👥
            </text>
            <text x='600' y='160' textAnchor='middle' fill='white' fontSize='20'>
              👤
            </text>
            <text x='300' y='310' textAnchor='middle' fill='black' fontSize='20'>
              👥
            </text>
            <text x='500' y='290' textAnchor='middle' fill='white' fontSize='25'>
              🌟
            </text>
          </svg>
        ...
        >
          <div className={styles.testimonialCard}>
            ...
            <p className={styles.testimonialText}>
              I met my co-founder at a conference using uplayground. The Green Light system made it easy
              to know who was open to networking, and the icebreaker suggestions helped start a
