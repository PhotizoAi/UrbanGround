...
export const Analytics = (): null => {
  useEffect(() => {
    ...
    const existingScript = document.querySelector('script[data-domain="uplayground.gg"]');
    if (existingScript) return;

    // Create and inject Plausible script
    const script = document.createElement('script');
    script.defer = true;
    script.setAttribute('data-domain', 'uplayground.gg');
    ...
    return () => {
      const scriptToRemove = document.querySelector('script[data-domain="uplayground.gg"]');
      if (scriptToRemove) {
